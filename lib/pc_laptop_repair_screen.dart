...
class PcLaptopRepairScreen extends StatefulWidget {
  const PcLaptopRepairScreen({super.key});

  @override
  State<PcLaptopRepairScreen> createState() => _PcLaptopRepairScreenState();
}

class _PcLaptopRepairScreenState extends State<PcLaptopRepairScreen>
  ...
  Future<void> _logTutorialToHistory(NormalizedTutorial tutorial) async {
    ...
    try {
      ...
      debugPrint("Failed to log tutorial to history: $e");
    }
  }
  ...
  void _listenToSavedLists() {
    ...
    _savedListener = _firestore.collection('users').doc(user.uid).snapshots().listen((snap) {
      ...
      if (data != null && data['savedPcLaptopTutorials'] != null) {
        setState(() {
          _savedTutorialIds = List<String>.from(data['savedPcLaptopTutorials']);
        });
      }
    });
  ...
  Future<void> _toggleSaveTutorial(NormalizedTutorial tutorial) async {
    ...
    if (_savedTutorialIds.contains(safeId)) {
      ...
      await userDoc.set({
        'savedPcLaptopTutorials': FieldValue.arrayRemove([safeId])
      ...
      await userDoc.set({
        'savedPcLaptopTutorials': FieldValue.arrayUnion([safeId])
      }, SetOptions(merge: true));
  ...
  Future<void> _determinePositionAndFetchShops() async {
    ...
    try {
      ...
      await _fetchNearbyPcShops(pos.latitude, pos.longitude);
    } catch (e) {
  ...
  Future<void> _fetchNearbyPcShops(double lat, double lon) async {
    ...
    final query = """
      ...
      (
        ...
        node["craft"="electronics_repair"]["repair:computer"="yes"](around:$radius,$lat,$lon);
      );
      out center;
    """;
    final url =
    ...
    try {
      ...
      for (var e in elements) {
        ...
        final name = tags['name'] ?? 'PC Repair Shop';

        final isPcRepair = tags.containsKey('shop') && tags['shop'] == 'computer' ||
            tags.containsKey('service') && tags['service'] == 'computer_repair' ||
            (tags.containsKey('craft') && tags['craft'] == 'electronics_repair' && tags['repair:computer'] == 'yes');

        if (!isPcRepair) continue;
      ...
      debugPrint('Error fetching PC shops: $e');
    }
  }
  ...
  Future<void> _toggleSaveShop(Map<String, dynamic> shop) async {
    ...
    final ref =
    _firestore.collection('users').doc(user.uid).collection('saved_pc_shops').doc(id);
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('PC & Laptop Repair',
              style: TextStyle(fontWeight: FontWeight.bold)),
          bottom: const TabBar(
            tabs: [
              Tab(icon: Icon(Icons.menu_book), text: 'Tutorials'),
              Tab(icon: Icon(Icons.computer), text: 'Nearby Shops'),
            ],
          ),
        ),
        body: TabBarView(
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Search PC & Laptop tutorials (fan, keyboard, etc.)',
              filled: true,
  ...
  Widget _buildShopsTab() {
    return _loadingShops
        ...
        ? const Center(child: Text('Enable location to find nearby PC shops'))
        : Stack(
      children: [
        GoogleMap(
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        final s = _shops[i];
                        final saved = _savedShopIds.contains(s['id']);
                        return ListTile(
