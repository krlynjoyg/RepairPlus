...
class MacImacRepairScreen extends StatefulWidget {
  const MacImacRepairScreen({super.key});

  @override
  State<MacImacRepairScreen> createState() => _MacImacRepairScreenState();
}

class _MacImacRepairScreenState extends State<MacImacRepairScreen>
  ...
  void _listenToSavedLists() {
    ...
    _savedListener =
        _firestore.collection('users').doc(user.uid).snapshots().listen((snap) {
          ...
          if (data != null && data['savedMacTutorials'] != null) {
            setState(() {
              _savedTutorialIds = List<String>.from(data['savedMacTutorials']);
            });
          }
        });
  ...
  Future<void> _toggleSaveTutorial(NormalizedTutorial tutorial) async {
    ...
    if (_savedTutorialIds.contains(safeId)) {
      ...
      await userDoc.set({
        'savedMacTutorials': FieldValue.arrayRemove([safeId])
      ...
      await userDoc.set({
        'savedMacTutorials': FieldValue.arrayUnion([safeId])
      }, SetOptions(merge: true));
  ...
  Future<void> _determinePositionAndFetchShops() async {
    ...
    try {
      ...
      await _fetchNearbyMacShops(pos.latitude, pos.longitude);
    } catch (e) {
  ...
  Future<void> _fetchNearbyMacShops(double lat, double lon) async {
    ...
    try {
      ...
      for (var e in elements) {
        ...
        final name = tags['name'] ?? 'Mac Repair Shop';
        final isComputerRepair =
            (tags['shop'] == 'computer') ||
                (tags['service'] == 'computer_repair') ||
                (tags['repair:computer'] == 'yes');

        if (!isComputerRepair) continue;
  ...
  Future<void> _toggleSaveShop(Map<String, dynamic> shop) async {
    ...
    final ref =
    _firestore.collection('users').doc(user.uid).collection('saved_mac_shops').doc(id);
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('Mac & iMac Repair',
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Search Mac or iMac tutorials (battery, screen, etc.)',
              filled: true,
